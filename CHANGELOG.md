...
## v0.62.1-fast.4 — release candidate

Compatibility and correctness rollup based on upstream quic-go v0.62.0. Go 1.26.0 remains the minimum; the declared module path and dependency versions are unchanged from fast.3. The [release record](docs/releases/v0.62.1-fast.4.md) defines the candidate's compatibility, support and validation boundaries. This entry selects the next version; it does not assert that the tag is already published.
...
- Optional transport qlogs created in the same second now use exclusive owner-only files instead of truncating one another. `TransportError.Unwrap` omits nil members while preserving `net.ErrClosed`, crypto-error identity and `errors.Is` / `errors.As` behavior. [#419](https://github.com/the-sarge/quic-go-fast/pull/419), [#466](https://github.com/the-sarge/quic-go-fast/pull/466)
...
The maintained test fixtures now join more of their workers, retain bounded failure captures and use deterministic ownership assertions. Those repairs improve validation but do not resolve the documented intermittent macOS failures or establish arbitrary-loss recovery, general performance improvement, or new native offload support.
