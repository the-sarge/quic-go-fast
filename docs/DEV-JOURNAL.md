...
## Windows coalesced receive landed (W3); Track W complete - 2026-09-12 02:30 EDT

**Main:** `b76174c94a55`
**Actor:** Claude

Merged [PR #248](https://github.com/the-sarge/quic-go-fast/pull/248) as `b76174c9`, closing [issue #233](https://github.com/the-sarge/quic-go-fast/issues/233) — Slice W3 of the [Windows datapath plan](adr/2026-09-11-windows-datapath-plan.md), completing Track W of datapath offload program QGF-DP-2026-09. Windows transport-owned sockets gain coalesced receive (URO): `isUROEnabled` sets `UDP_RECV_MAX_COALESCED_SIZE` (the setsockopt is the probe, mirroring the Linux `UDP_GRO` probe) on transport-owned sockets only and honors the existing `QUIC_GO_DISABLE_GRO` kill switch; `newConn` wires the result into the platform-neutral `connCapabilities.GRO` flag; and `windowsConn.ReadPacket` posts a 64 KiB coalesced-tier buffer per read, parses `UDP_COALESCED_INFO` in the WSAMSG control-message walk, and splits each non-empty read through G1's shared `coalescedSlab` into per-datagram views returned one per call via a `pendingSegments` queue (single datagrams slabbed too, keeping retention copies uniform), with `releaseReadBuffers` releasing undelivered views when the read loop exits. This closes G1's inert seam for its second producer; the platform-neutral retention wiring (`retainForRetentionQueue`) activates automatically with no queue-side code change. Caller-supplied sockets are never probed and keep the W1/W2 behavior. As the W track's second lander, W3 also delivers the four-combination USO/URO interaction matrix. Shipped URO code is unchanged from the slice's first commit through merge (`git diff 559cc827..6b480b91 -- sys_conn_windows.go` empty; the only later change is a doc comment).

### Decisions

The adoption gate ran in two phases. The v1 collection on the hosted `windows-latest` runner dispositioned **Fail** with 0 % engagement across ~7.8 M datagrams; diagnostics proved the cause structural, not noise — Windows delivers same-host traffic through an internal shortcut that bypasses the NDIS receive-indication path where coalescing happens, so no single-host surface (loopback or own-NIC address, any bind order or backlog, GSO or individual sends) can exercise URO, and the hosted runner has no second endpoint reachable to it. This surfaced as a stop-for-decision. The operator directed requalification on owned infrastructure. Using the `infra` project's Windows Server 2025 KVM golden, a transient guest on `minimax` received bulk QUIC across the libvirt virtual NIC from the minimax Linux host — a real NDIS receive path. The protocol was amended to **v2** (new qualified surface, two-process cross-host harness; no cell/metric/bound/disposition change) before any counted collection, and the v1 record is retained as history. The program's qualified Windows surface for receive-offload engagement is now a two-endpoint KVM guest, not the hosted runner ([protocol](audits/2026-09-11-w3-uro-protocol.md), [results](audits/2026-09-11-w3-uro-results.md)).

### Validation

Adoption ([results](audits/2026-09-11-w3-uro-results.md), disposition **Pass**): v2 collection on the minimax KVM virtual-NIC surface (guest build 10.0.26100, 4 vCPU, go1.27.1, amd64; Linux sender pinned `taskset -c 12,13,28,29`), 11 rounds × 4 paired same-round cells, round 0 discarded, uncontaminated, cell integrity clean. Primary — receive syscalls per delivered datagram, engaged vs disabled — geometric mean 0.098, 95 % CI [0.0977, 0.0990] against the ≤ 0.75 gate (~10.2× fewer). Engagement 95.7 % of datagrams coalesced. Throughput 1.34× [1.17, 1.53] against the 0.95 noninferiority gate. Peak-working-set delta −0.3 MB, within the +8 MiB budget. One recorded deviation: the unexercised cell (sender GSO off) still coalesced ~95.8 %, because Windows URO merges any fast same-flow stream regardless of sender segmentation — non-informative here, and the mechanical rule does not gate on it.

URO closure classes and the four-combination interaction matrix (`TestWindowsUSOUROInteractionMatrix`) pass on the KVM guest at the candidate head under strict CI host posture; the Linux `-race` coalesced routing-closure fixtures (G1 shared, the W3 concurrency owner) pass. Local certification at ready head `fd6c77bb`: cross-compile builds (windows/linux/darwin × amd64/arm64 + windows/386), `golangci-lint` 0 issues. All 33 hosted checks — including Windows unit tests on Go 1.26.x and 1.27.x, the authoritative full-suite surface — passed on `fd6c77bb`; squash-merged with `--match-head-commit`.

Review: RAS run `20260912T055332-9cad6ab69dfdfefd262e1218` (initial, briefed from the exact plan slice) returned 7 low-severity Fix First clusters, all documentation/test-accuracy (provenance, four-combination evidence map, arithmetic, capability-skip ordering, ownership comment). All dispositioned `fix-now` and fixed at `754ca0b4`; exact-head verification reported all resolved. Two new low doc-accuracy concerns (a `git diff` ref stale at HEAD, a preclaimed merged-head CI receipt) were fixed docs-only at `fd6c77bb` with the RAS rerun skipped under the shared low/nit docs-only policy. C-003 (negative tests on a URO-incapable Windows build) is deferred and filed as a follow-up: not reproducible without such a host, and the probe's failure branch is already covered deterministically by the fake `RawConn` in `TestWindowsUROProbeFailure`.
...
## Retained HTTP failure diagnostics installed - 2026-09-14 15:28 EDT

**Main:** `47a2e96ff9a9`
**Actor:** Codex

Merged [PR #312](https://github.com/the-sarge/quic-go-fast/pull/312) as `47a2e96ff9a91cfbfec575b22b280d9ebae6f5d4`, installing retained diagnostics for the HTTP idle-timeout and reconnection fixtures. Paired endpoint events, custom dial attempts, handshake/recovery progress, HTTP timer observations and request milestones remain captured after early dial return, with an explicit pre-teardown boundary. The integration workflow retains checksummed command output and source/environment metadata within a shared 256 MiB quota and 14-day artifact retention. Passing intact fixture captures release their reservation; incomplete evidence is reported. The [maintained contract](agents/http-failure-captures.md) records lifetime, provenance, storage and observer limits.

Controlled failure subprocesses demonstrated attributable retained evidence for both fixtures. Native Go 1.27.1 darwin/arm64 focused race checks passed in 2.328s; uncached HTTP/3 and self-suite checks passed in 1.101s and 25.346s. Vet, lint, module tidiness, formatting, documentation links and workflow syntax passed. Downloaded macOS artifacts verified full command-output retention and SHA-256 checksums. All 33 hosted checks succeeded on candidate `28c9499830ab58d7122773f25b6a10722820cabf`; one Linux job needed the documented infrastructure-only rerun after every test passed but artifact creation failed with `ECONNRESET`.
...
## External Windows segmented sends landed - 2026-09-16 01:04 EDT

**Main:** `5c20d9eb6eb0`
**Actor:** Codex

Merged [PR #367](https://github.com/the-sarge/quic-go-fast/pull/367) as `5c20d9eb6eb0aa7ea605135b07064c1962dd4124`, closing [Q02 / #326](https://github.com/the-sarge/quic-go-fast/issues/326). Windows now probes read-only USO support on external ordinary and explicitly registered sockets. Segmented submission continues through the supplied WriteMsgUDP wrapper; receive-coalescing permission and socket close ownership retain their existing owners. The product PR includes Q02 completion and the remaining Q03–Q05 frontier in the [normative plan](adr/2026-09-15-external-packet-io-plan.md#q02--enable-external-windows-segmented-sends).

Validation: a native Windows red regression first demonstrated the old ownership restriction. On reviewed head `25112982840323a6c42457d2cfc7b7ce63b120ef`, both Windows Go 1.26 and 1.27 passed all six external IPv4/IPv6, registered/ordinary, disabled and unavailable delivery cases without skips; Linux GSO passed ordinary and race executions. Local Go 1.27.1 root tests, native/Windows vet, module tidiness, Windows test compilation and exact-head clean-tree/diff checks passed. Hosted unit, integration, lint, cross-compilation and interop workflows all passed; [unit run 35056904427](https://github.com/the-sarge/quic-go-fast/actions/runs/35056904427) and the [validation receipt](https://github.com/the-sarge/quic-go-fast/pull/367#issuecomment-5692308826) retain the job identities and other run links.
...
## Linux managed receive normalization landed - 2026-09-16 12:46 EDT

**Main:** `3f4688c0ac21`
**Actor:** Codex

Merged [R01-L / PR #378](https://github.com/the-sarge/quic-go-fast/pull/378) as `3f4688c0ac21a2bba92fd854adf02cc993a44290`, closing [#332](https://github.com/the-sarge/quic-go-fast/issues/332). Linux managed registration now installs persistent endpoint normalization before enabling GRO. Public reads preserve individual datagrams, independently mutable source addresses and wrapper policy across lease generations; return joins active I/O, discards consumed QUIC storage and restores logical deadlines, with terminal cleanup on restoration failure. The product PR owns the committed completion/frontier transition.

[Exact-head certification](https://github.com/the-sarge/quic-go-fast/pull/378#issuecomment-5701139249) records candidate `9c997a65bd7a674e968891af2cd03d804eb0fc6d` against base `feba5306e6dbc68d126b9bc2adc5f39b6954d4ca`: native Linux arm64 IPv4/IPv6 queued-aggregate handback and filtering, focused race/lifecycle/storage/diagnostic tests, Linux root tests/vet, the full local suite with integration timing factor 3, vet, tidy and lint passed. All 33 hosted checks passed, including [unit](https://github.com/the-sarge/quic-go-fast/actions/runs/35122734240), [integration](https://github.com/the-sarge/quic-go-fast/actions/runs/35122734187) and [interop](https://github.com/the-sarge/quic-go-fast/actions/runs/35122734190). The [native receipt](audits/2026-09-16-r01-l-managed-receive.md) records the finite evidence, source reuse and initial invocation corrections; no performance improvement is claimed.
...
## Darwin 27 arm64 batching enabled - 2026-09-22 18:06 EDT

**Main:** `dddf8b8cecf1`
**Actor:** Codex

Merged [PR #525](https://github.com/the-sarge/quic-go-fast/pull/525), enabling the existing `sendmsg_x` path on Darwin 27 arm64. The product change is one architecture-scoped allowlist entry; Darwin 25 admission and the existing retry, latch, startup self-check and fallback behavior are preserved. Darwin 27 amd64 remains excluded.

The first qualification's missing transient-error evidence was resolved by a bounded native comparison: all 17 observations on each Darwin 25/27 host matched on their first run. Real IPv4/IPv6 EAGAIN and EINTR tests checked zero progress and accepted prefixes with exact peer payloads and ECN. AF_UNIX ENOBUFS controls remain labeled separately. Inspection of the exact running Darwin 27 kernel, matched by UUID/digest and mapped through its syscall table, established protocol-independent batch-count suppression including ENOBUFS. This does not claim native UDP ENOBUFS induction or whole-kernel equivalence. The [results and evidence](audits/2026-09-22-darwin27-errors/results.md) supplement the unchanged historical qualification under [ADR 0008](adr/0008-darwin-batch-compatibility.md).

Final production gates passed 22 test/subtest cases and three opt-out cases without skips; Darwin arm64/amd64 builds, vet and tidy passed. Exact-head certification on `4df8adbe19676947950053f0b8bef2815fbbb91e` checked the shipped architecture table, clean tree and diff whitespace; all 33 hosted checks passed before merge. Review strengthened the existing admission test to use the production table, with an architecture-bypass mutation rejected. A CI failure in the existing dual-stack ECN test exposed waits that ignored the configured timeout scale; its two waits now honor that scale. A bounded delayed-channel diagnostic verified the premature timeout and repair, without claiming to reconstruct the original runner's scheduling.
