...
| Stream opening/header/pre-response failure | Abort owned stream activity and request input; finish after any started uploader exits. | Named slice owner; regression/characterization required before completion |
...
| Retry | Each attempt has a separate balanced obligation; no lease transfer to the successor. | Named slice owner; regression/characterization required before completion |
