...
| D02 | Enable offloads through the retained wiremux wrapper; P02 has selected retention | Native fixed-peer filtering remains an independent fork API, not a later wiremux removal task |
...
Performance: reuse the fork's existing native adoption receipts for unchanged algorithms; Q02–Q05 and R01-L/W add focused correctness, observed engagement and lifecycle evidence for changed paths. Retire E01-L/W/D standalone baseline campaigns. E02-L/W/D each run one final matched control/candidate comparison through the retained wiremux wrapper: reliable, Datagram and mixed paced workloads, ten paired rounds each, 5 s warmup and 10 s measurement. Use the current wiremux E01 protocol for identities, admission, metrics and termination. There is no mandatory capacity pilot, impairment campaign, TURN performance harness or socket-class cross-product. Representative TURN, managed reuse and compatibility correctness remains required.

The final comparison can recommend a default only with at least 10% lower combined endpoint CPU per delivered byte, a paired 95% interval supporting that threshold, and preservation across all three cells: no correctness failure or new unexplained delivery deficit, and upper paired bounds below 5% regression for applicable p99 latency and sampled resident memory. Inconclusive resource/timing/quantile evidence cannot pass that gate, but a correct natively qualified capability may finish as opt-in with ordinary defaults and explicit performance limitations. Demonstrated regressions or absent native qualification block the affected capability. Keep one unchanged-code campaign; any extra campaign requires a material fix or distinct recorded causal question and a committed finite amendment. No search for passing numbers is authorized. P02 is settled wrapper retention; its historical experiment is not rerun or reinterpreted by this policy.
