...
The [W1 native receipt](../audits/2026-09-23-windows-ecn-w1/README.md) establishes IPv4-only and IPv6-only feasibility on Windows Server 2025 build 26100.32230/amd64, Go 1.27.0 and x/sys 0.48.0. `IP_RECVECN` and `IPV6_RECVECN` enable native integer `IP_ECN` and `IPV6_ECN` control messages carrying all four received codepoints; an independent Linux peer observes the requested Not-ECT, ECT(0) and ECT(1) outgoing marks. Winsock rejects application-generated CE. Successful probe Go `WriteMsgUDP` calls return zero payload bytes on this surface despite complete peer-observed delivery, including calls without ECN OOB. The receipt distinguishes this Go completion-path observation from general Winsock semantics; the later implementation contract must address send-result handling. W1 introduced no managed runtime capability. The [Windows successor plan](2026-09-22-windows-managed-ecn-plan.md) requires native Windows 11 and Server 2025 qualification, including dual-stack/mapped sockets, before one complete managed implementation slice. Runtime availability remains per-socket and per-family, with ordinary fallback on option denial; it does not use an OS-edition/build allowlist. Older releases remain outside the native qualification claim. The W1 receipt itself introduces no runtime or public/raw authority.
