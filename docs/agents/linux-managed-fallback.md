...
The approved blast radius is Linux managed receive setup, minimal internal error classification in the shared decoder, focused tests and this receipt/contract. Shared decoder callers retain their error text and failure behavior. No public API, descriptor exposure, close owner, receive decoder, dependency or CI change is authorized. Wildcard packet-info failure must not be hidden behind an earlier ECN error. No performance, fuzz, extra-platform or open-ended restriction campaign is authorized; existing hosted coverage remains applicable.
