...
The owner is the handshake test fixture. A failure report identifies the requested direction, loss pattern, retry, speaking order, post-quantum setting, certificate-chain setting, and QUIC version without renaming existing subtests. It retains the latest client/server helper phases separately from a bounded tail of router decisions and existing qlog events. Actual packet directions are recorded separately from the requested direction, since the historical one-third callback still applies bidirectionally. The direction correction remains owned by [issue #79](https://github.com/the-sarge/quic-go-fast/issues/79).
