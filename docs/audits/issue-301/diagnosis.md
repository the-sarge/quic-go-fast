...
The historical second-request timeout was not reproduced, and its cause remains unresolved. Preserved evidence combined with unchanged source supports a narrower boundary inference: a fresh client QUIC connection timed out before handshake completion. The direction and component that stopped making progress remain unknown. One instrumented seeded suite replay and one isolated-test experiment passed on 2026-09-14 UTC. The two conditional experiment slots remain unused: no failure or concrete predecessor signal justified further minimization. This completes the approved diagnosis, not a repair or closure of [#301](https://github.com/the-sarge/quic-go-fast/issues/301).
...
The ranked observations were: (1) fresh QUIC connection establishment, because different HTTP fixtures reach the same dial path; (2) failed-entry eviction/fresh dialing specific to #301; (3) concrete prior-test interference. These were priorities for falsifiable observations, not ranked causal conclusions. The source-based analysis below excludes simple reuse of the deliberately failed entry as the explanation for the historical timeout; instrumenting the callback still supplied the requested direct attempt milestones. There was no additional dedicated #151 experiment; its ordinary inclusion in the approved #301 full-suite replay did not reopen its exhausted investigation.
...
The concrete connection is shared implementation reachability: #151 and #301 use a custom callback calling [DialAddrEarly](https://github.com/the-sarge/quic-go-fast/blob/f0c7afd23dd2355110453a8cd1bcc1f5996c6f77/client.go#L44); [HTTP/0.9](https://github.com/the-sarge/quic-go-fast/blob/f0c7afd23dd2355110453a8cd1bcc1f5996c6f77/interop/http09/client.go#L93) also calls it. This path resolves the address, creates a socket-backed transport, and enters QUIC dialing. #301's observed server acceptance before handshake completion also matches a successful observation in #151, but with the HTTP idle timer disabled.

**Source-derived inference, not a captured historical handshake event:** the preserved error string is produced by [IdleTimeoutError](https://github.com/the-sarge/quic-go-fast/blob/f0c7afd23dd2355110453a8cd1bcc1f5996c6f77/internal/qerr/errors.go#L92), emitted at [the connection idle-timeout branch](https://github.com/the-sarge/quic-go-fast/blob/f0c7afd23dd2355110453a8cd1bcc1f5996c6f77/connection.go#L706). The fixture leaves both endpoints' QUIC timeout configuration unset; defaults are five seconds for handshake idleness and 30 seconds for post-handshake idleness. After handshake completion, the deadline is at least the negotiated 30-second idle period (`max(idleTimeout, 3*PTO)`); the whole failing test took only 5.00s. Under this unchanged source/configuration, the error therefore identifies the client connection's pre-handshake idle-timeout branch. The ten-second absolute handshake deadline has different error text. This reasoning uses the source path and both endpoints' configuration together with the preserved log, not elapsed time alone. `config.go`, `internal/protocol/params.go`, and `internal/qerr/errors.go` are also unchanged from the failed synthetic merge.

The first custom dial returns the deliberate sentinel without creating a QUIC connection. [Failed-entry handling](https://github.com/the-sarge/quic-go-fast/blob/f0c7afd23dd2355110453a8cd1bcc1f5996c6f77/http3/transport.go#L344) removes that entry and would return its stored sentinel if encountered again; it cannot turn the sentinel into the reported idle timeout. Thus the historical error supports fresh real dialing and a client connection reaching its run loop, rather than simple reuse of the deliberately failed cache entry. This does not identify whether Initial or Handshake traffic was unsent, lost, unprocessed, or followed by an unobserved peer close; it establishes no repair or causal regression seam.
...
The smallest missing observation is one natural occurrence of the exact second-GET failure with paired endpoint capture: dial attempts/results and effective configuration; listener and resolved destination; Initial/Handshake packet progress, including probe-timeout (PTO) scheduling and retransmission observations; server receipt/acceptance and both handshake signals; request stream ID/handler entry; response header/write/FIN progress; and closure causes before versus during teardown. Bind it to tested SHA, instrumentation patch, toolchain, platform, command, timescale, version, seed, and complete checksummed log. Include event counts, truncation and encoding errors. Missing connection events alone cannot distinguish packets never arriving, being dropped before demultiplexing, or incomplete capture.
