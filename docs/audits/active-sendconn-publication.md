...
| `Conn.handleHandshakeComplete`, `Conn.handleHandshakeConfirmed`, `Conn.handleUnpackedLongHeaderPacket`, `Conn.prepareEmission` | Connection goroutine reads capabilities or addresses directly; serialized with the sole writer. |
| `packetEmission.sendAny`, `packetEmission.rebindPath`, `packetEmission.close` | Connection goroutine reads the slot directly for live capabilities, atomic remote-address rebinding, or the final synchronous write. |
