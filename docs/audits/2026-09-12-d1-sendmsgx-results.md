...
- **Candidate head:** `1af9bb81` (harness commit on the D1 branch; the shipped code at collection time was unchanged since `4649c463`). Base `5c0a459c`. Provenance note: review-fix commits landed after collection (`d23eaad4` and successors) and changed the errno-classification, batch-eligibility, and zone-resolution branches; the collected cells exercised the native-UDP success path, which those fixes preserve. The inert-cell assertions, closure suites, and the ordinary test suite were re-run at the final PR head; the success-path measurements were not re-collected, per this protocol's fixed evidence budget.
