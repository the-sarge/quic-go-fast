...
Initial assessment accounting: 356 Go source files; 2550 maintained declarations; 2485 initial-screen, 31 source-inspected and 34 finding-linked entries. These statuses are mutually exclusive per declaration. Ten maintained Go files without function bodies also have file-level initial assessments. Additional non-Go/module/data infrastructure files: 40. File classifications and per-kind counts are in the inventory.

Non-Go infrastructure selection includes tracked workflows and their adjacent scripts, ClusterFuzzLite configuration, local hooks, linter and coverage configuration, root/nested maintained module manifests and checksums, interop container/endpoint setup, fixture testdata, and the nested vendor fixture ignore rule. Production/example program bodies and frozen archive inputs retain the exclusions below; unrelated repository governance and prose are outside the execution-infrastructure census. `interop/Dockerfile` uses mutable base images and downloads/builds the configured toolchain; `codecov.yml` configures reporting exclusions/status thresholds, not a test oracle; module checksums bind dependency content but do not establish behavior; the vendor ignore rule excludes generated vendor output. All receive initial assessments with deeper external-service/consumer questions deferred.

The inventory records explicit `//go:build` expressions separately from implicit Go filename GOOS/GOARCH suffix constraints, including suffix-only Linux and Windows files. `nonlinux` and `unix` are not treated as GOOS suffixes; their explicit build expressions own selection. File-level infrastructure assessment arrays use the same seven-dimension order as member arrays, with prose identifying locally inapplicable execution dimensions. Member values are screen-level unless explicitly inspection-backed; `Q-sensitive` is selected by worker, sleep, direct-random, polling or conditional-skip signals. `Q-worker-callback` identifies seven additional connection-mock groups requiring exact caller-context inspection; it is not a claim that all callbacks escape.
