...
CELLS = [
    dict(cell='P1', mode='datagram', rate=1_000_000_000, processors=4, streams=1, gso=True),
    dict(cell='P3', mode='datagram', rate=4_000_000_000, processors=4, streams=1, gso=True),
    dict(cell='P4', mode='datagram', rate=4_000_000_000, processors=4, streams=1, gso=False),
    dict(cell='P5', mode='stream', rate=0, processors=4, streams=1, gso=True),
    dict(cell='P6', mode='stream', rate=0, processors=4, streams=16, gso=True),
]
...
def main():
    ...
    parser.add_argument('--cells',default='P1,P3,P4,P5,P6')
    args=parser.parse_args()
    if args.output.exists():
        raise SystemExit('refusing to overwrite a capture')
    args.output.mkdir(parents=True)
    cells=[c for c in CELLS if c['cell'] in args.cells.split(',')]
    frozen=dict(campaign_role="remaining replacement; four-core contract", launcher_sha256=hashlib.sha256((ROOT/"isolate.sh").read_bytes()).hexdigest(), cells=cells,paired_samples=1 if args.smoke else 10,measurement_seconds=3 if args.smoke else 60,
                ...
                base='e742e3ee64c79c61440061a86f56e2e223a776ef',candidate='8e61c8e8b9997204316d6009b9065943a5e8b82f')
