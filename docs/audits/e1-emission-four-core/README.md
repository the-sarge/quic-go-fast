...
**Disposition: inconclusive; no runtime adoption.** The complete four-core replacement produces numerical bounds within the agreed margins for P3/P4/P5/P6, while P1's successful-probe p99 bound and the existing stream-churn time bound exceed 1.05. Review also established that capture tooling could execute on measured endpoint cores, so this campaign does not establish dedicated-core qualification; numerical margin passes are not fully qualified passes. Every scheduled capture completed successfully; the two missed bounds are statistical qualification failures, not discarded samples or diagnosed regressions of that size. The revised interface and focused allocation provenance are complete under the finite E1 contract. This receipt exhausts the remaining campaign and diagnostic budgets authorized by the [resumption contract](../../adr/2026-09-07-packet-emission-plan.md#e1--establish-packet-emission-feasibility) and [resumption receipt](../2026-09-07-emission-four-core-resumption.md). The [original E1 receipt](../e1-emission/README.md) remains unchanged. E2 remains independently ready; E3–E6 remain blocked pending a new feasibility decision. Two-core performance has no acceptance role.
...
The [preflight archive](preflight.tar.gz) retains allocation logs/manifests, one paired three-second setup run for each of the five four-core cells and the separate qlog-on paired P3 diagnostic. All setup samples passed the frozen payload, timeout, ledger, capability and sampled guest/SMT threshold checks; the shared capture-tool placement limitation below also applies to setup. Qlog-on missed schedules remain recorded and are not included in the qlog-off qualification. [Build logs](build-logs.tar.gz) retain the failed baseline suite as well as the passing remaining checks. The [host receipt](host.json) binds machine/toolchain identity and capture-source/binary hashes before the replacement campaign.

## Four-core qualification

The complete replacement ran on minimax from 14:39 to 16:26 UTC on 2026-09-07: five cells, ten adjacent alternating baseline/candidate pairs per cell, 2 s warmup, 60 s measurement and 1 s drain. All cells use four physical cores and GOMAXPROCS=4 per endpoint, with client cores 8–11, server cores 12–15 and unused SMT siblings 24–31. Unrelated work in user.slice, system.slice and machine.slice was restricted to cores 0–7 and 16–23 during capture. The separate capture service itself had AllowedCPUs=8–15, including the Python collector and its unpinned mpstat child, so those tools could execute on endpoint cores. The observed guest/SMT thresholds do not measure this same-core tooling contribution. Dedicated-core qualification is therefore not established, and no measured effect size is inferred for that contribution. Separate endpoint processes communicate over loopback UDP, using QUIC v1 and qlog off; GSO is enabled except P4. These observations do not qualify a physical link or non-Linux performance. The frozen analyzer's per-cell `pass` values denote numerical comparisons only; the placement limitation prevents promoting them to full host-qualified passes.

All 100 samples passed payload, duplicate, probe-timeout, ledger, GSO-capability and the implemented guest/SMT threshold checks. These finite checks do not discharge the separate dedicated-core requirement. The [host summary](campaign-02/host.json) records zero sampled guest CPU and maximum per-run mean SMT-sibling busy time of 0.13984%, below the frozen 0.1% guest and 1% sibling limits. The [restoration receipt](campaign-02/restoration.json) records exit 0, initially unrestricted CPU settings restored, reservation marker removed and backup timer stopped. The [manifest](campaign-02/manifest.json) binds workload, source and launcher identity; the [raw archive](campaign-02/raw.tar.gz), SHA-256 `d511a7617f6ea0fffe9b65eafc6f3deb14ca4fb379cd3af180731daa3f2f11f8`, retains every sample, invocation, endpoint log, CPU observation, capture log and restoration record.
...
| Cell, four cores per endpoint | Goodput, base / candidate Gbps | Goodput lower bound | CPU/unit upper bound | Bytes/unit upper bound | Probe p99 upper bound | Missed/failed upper difference, percentage points | Numerical margin |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
...
Goodput values are geometric means of whole runs. Bounds are candidate/base ratios except the last numeric column. The numerical margins require goodput at least 0.95, CPU/allocated bytes/probe p99 at most 1.05 and missed/failed proportion difference at most +0.1 percentage point. P1's geometric p99 ratio is 0.9494, while its upper bound is 1.079320; the central estimate is lower, but the ten pairs do not exclude more than 5% worse p99. P5, which missed its historical two-core margin, has a four-core p99 upper bound within the numerical margin at 1.035728; its host qualification remains unestablished. This does not retroactively change any historical receipt.
...
The revised concrete result interface and complete allocation provenance address the authorized gaps, and P3/P4/P5/P6 meet the numerical bounds at the intended four-core budget. Overall feasibility remains inconclusive because dedicated-core qualification is unestablished and P1 p99 and stream churn also miss their numerical margins. All finite diagnostic, setup, focused-allocation and qualifying samples are retained. The replacement allowance is exhausted; no third campaign, changed margin or runtime adoption is implied. E2 remains the independently ready frontier. E3 and its descendants require a new feasibility decision before runtime migration can proceed, in addition to their existing slice dependencies.

The remaining fresh replacement review completed under the accepted E1 contract. [Independent dispositions](review.md) record the placement correction, rejection of the missing-drain claim and reviewer/adjudicator failures. Final exact-head local/hosted certification is recorded on [PR #37](https://github.com/the-sarge/quic-go-fast/pull/37); no receipt claims a future head has already been certified.
