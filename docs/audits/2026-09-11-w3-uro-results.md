...
- **Candidate:** the measured shipped URO code and harness are the build committed at `6b480b91` (PR #248). Shipped URO code (`sys_conn_windows.go`) was unchanged from the slice's first code commit through the measured candidate — `git diff 559cc827..6b480b91 -- sys_conn_windows.go` is empty — so the measured shipped behavior equals the reviewed shipped behavior. The measured **harness** is the ephemeral-certificate two-process build; the intermediate `7eddc0f8` harness used a deterministic-certificate scheme that could not complete a run (Go 1.27 hedged ECDSA signing makes a regenerated certificate non-reproducible across the two processes), so it was replaced with an ephemeral certificate plus SHA-256 fingerprint handoff before any counted collection. That change touched only harness identity, not the shipped URO code or the measured receive behavior (`git diff 7eddc0f8..6b480b91 -- w3_measurement_*.go` is confined to the three harness files). A later review-fix commit (`6b480b91..754ca0b4`) adds only the `windowsConn` ownership doc comment to `sys_conn_windows.go` and reorders a capability check in the interaction test — no change to the measured URO datapath — so the `6b480b91` measurement receipt still describes the shipped behavior at the ready head.
