...
**Disposition: inconclusive; no runtime adoption.** The bounded normal/GSO prototype preserves the characterized behavior and has matching recorded focused-allocation counts, but the initial full campaign does not establish the required latency preservation in P2 and P5, and the stream-churn benchmark misses its time margin. The result-shape and allocation-provenance limits below also prevent treating this as a complete positive qualification. This PR retains ordinary characterization, opt-in evidence and an inert patch only. E3–E6 remain blocked pending a scoped re-handoff; E2 remains independently ready. The [normative E1 contract](../../adr/2026-09-07-packet-emission-plan.md#e1--establish-packet-emission-feasibility) governs; this receipt does not amend its performance margins.
...
The result shape is only partially exercised: the prototype sets `progress` and `stop`, but `Conn.sendPackets` consumes only `deadline` and `err`. Capacity resumption remains in `Conn.run` through `sendQueue.Available()`; the result does not carry a capacity wakeup. Non-pacing inner-loop outcomes other than `SendAny` collapse into `emissionRecoveryBlocked`, with no congestion-specific stop value. Top-level send-mode dispatch remains unchanged. Thus this experiment does not establish the full conceptual result contract, including distinct congestion and hard-blocking outcomes, as a consumed connection/module interface. These are limitations of the frozen prototype, not a reason to expand it after measurement.
...
The [raw archive](campaign-01/raw.tar.gz) retains every sample's counters, endpoint output, invocation and CPU observations plus the capture log and restoration receipt; SHA-256 `0a3f0b8a314eb1b82f0646eadf09fc96a783ac92ae63730d4a18eee40c4b5c89`. The compact [paired analysis](campaign-01/summary.json) is directly inspectable; individual sample receipts remain in the archive. To recompute, extract the archive into an empty directory and run `python3 analyze.py /path/to/extracted/campaign-01 --output /path/to/summary.json`. The analysis uses 20000 whole-pair bootstrap resamples, seed 20260907, geometric paired ratios and one-sided 95% percentile bounds. No packet is treated as an independent experimental unit.
...
The DATAGRAM rate labels are scheduled offered rates. The generator feeds a bounded fixture queue, and accepted traffic is materially lower: mean base ingress-drop fractions are 27.0% in P1, 26.9% in P2, 7.0% in P3 and 24.1% in P4. These pre-admission drops are not transport loss. They remain part of the recorded workload and do not independently invalidate its paired bounds or identify the underlying bottleneck.

The following are arithmetic means across ten whole runs for each cell/variant. P1–P4 ledger units are 1071-byte DATAGRAM messages; P5/P6 units are bytes. Probe p50 is the arithmetic mean of the ten successful-probe run medians, not a pooled-packet percentile.

| Cell / variant | Offered | Admitted | Delivered | Ingress drop | Post-admission drop | Probe p50, ms |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| P1 base | 7,002,801.0 | 5,112,925.1 | 5,112,630.7 | 1,889,875.9 | 294.4 | 1.304 |
| P1 candidate | 7,002,801.0 | 5,048,749.5 | 5,048,441.0 | 1,954,051.5 | 308.5 | 1.244 |
| P2 base | 28,011,204.0 | 20,488,872.5 | 20,481,687.6 | 7,522,331.5 | 7,184.9 | 1.955 |
| P2 candidate | 28,011,204.0 | 20,531,819.6 | 20,524,852.3 | 7,479,384.4 | 6,967.3 | 2.673 |
| P3 base | 28,011,204.0 | 26,042,748.1 | 26,042,697.3 | 1,968,455.9 | 50.8 | 1.198 |
| P3 candidate | 28,011,204.0 | 25,971,510.2 | 25,971,455.0 | 2,039,693.8 | 55.2 | 1.128 |
| P4 base | 28,011,204.0 | 21,264,959.1 | 21,256,502.7 | 6,746,244.9 | 8,456.4 | 2.247 |
| P4 candidate | 28,011,204.0 | 21,265,876.6 | 21,256,942.6 | 6,745,327.4 | 8,934.0 | 1.671 |
| P5 base | 70,129,313,382.4 | 70,129,313,382.4 | 70,129,313,382.4 | 0.0 | 0.0 | 0.262 |
| P5 candidate | 69,301,705,113.6 | 69,301,705,113.6 | 69,301,705,113.6 | 0.0 | 0.0 | 0.262 |
| P6 base | 73,461,445,427.2 | 73,461,445,427.2 | 73,461,445,427.2 | 0.0 | 0.0 | 0.320 |
| P6 candidate | 72,697,413,632.0 | 72,697,413,632.0 | 72,697,413,632.0 | 0.0 | 0.0 | 0.320 |

### Focused allocation provenance
...
The allocation fixture is [connection_emission_experiment_test.go](../../../connection_emission_experiment_test.go), introduced in product commit `cc23153d4eb056bcee98114a6e52cd49099b0796`; SHA-256 `5861ea9e4d7f0aa5117191104215bf34498b894dbeee70ac0405dc3cdd92431d`. It is absent from both frozen runtime commits and was copied untracked into their Linux worktrees. A post-capture check found both retained copies identical to this source. The logs in the preflight archive record the matching 37/104 observations and connection sizes, but do not bind the original shell command, source hash, Go version or outer GOMAXPROCS. The host checkpoint records Go 1.27.1, but it is not a per-invocation allocation receipt. This provenance limitation is retained; no missing metadata is inferred and no extra allocation measurement was taken to repair it.

For explicit reproduction with the retained fixture, `EMISSION_EXPERIMENT=1 go test -tags emission_experiment . -run '^TestEmissionFocusedAllocations$' -count=1 -v` selects the check. This is a reproduction command, not a claim that the exact original invocation was retained. The fixture calls `testing.AllocsPerRun(1000)` and records fixture-inclusive counts; its limited provenance cannot independently certify a positive allocation-preservation outcome.
...
Both failed and successful setup observations remain in the archive. The isolation launcher restored the initially unrestricted cgroup settings after each of these captures; the full campaign also restored its reservation on completion. The campaign ran before the launcher gained its separate `bench` mode. The retained campaign manifest and host checkpoint do not bind the campaign-time launcher bytes or their hash; they retain the service identity, per-sample CPU observations and restoration receipt. The currently committed launcher includes the later benchmark mode.

## Review and certification

The [independent review dispositions](review.md) record run `20260907T081528-7e4b074c7cec739bb0356bfc`, the receipt-only corrections and the mixed push/PR CI observations. The inconclusive outcome, inert prototype, E2 frontier and E3–E6 blocker state are unchanged. Final exact-head certification is recorded on [PR #33](https://github.com/the-sarge/quic-go-fast/pull/33); the historical failed push run is not represented as passing.
