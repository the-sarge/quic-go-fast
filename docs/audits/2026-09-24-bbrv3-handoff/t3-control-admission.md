...
Splitting the repair would leave reservation authority and its timer consumer disagreeing; merging a neighboring transport/controller slice adds no necessary behavior. Retain child #589 and the one product PR. Ten test functions remain the ceiling: extend the existing MTU case with due-ACK and no-ACK pending-work cases, retain migration/timer and active-isolation cases, and show isolation admission after pending bytes retire. No mutation, platform, timing-boundary campaign or repetition scope is added. Source context consists of the current T3/common contract, named design sections, four relevant helper/consumer ranges and the two named test functions; this receipt replaces complete review reports within the 3,000-token history allowance and leaves the 35,000-token dispatch ceiling intact.
