...
| Path-validation probes | `isPathProbePacket` returns before congestion and ECN send callbacks. The real probe is stored separately; ordinary history receives a zero-time placeholder that counts toward tracked length. If loss detection reaches its unacknowledged number, the placeholder can enter application-space `lostPackets` before the probe exclusion guard. | Filter placeholders as well as real probes from ordinary flight/delivery/loss sampling; do not infer valid sample metadata from diagnostic presence. Keep disposal separate from DPLPMTUD probes, which are different packets. [SentPacket][sent], [probe placeholder][history], [tracked length][history-length], [loss ordering][loss] |
...
| --- | --- |
| Sampler/accounting | Equal packet numbers in different spaces; equal timestamps; mixed sizes and retransmissions; DATAGRAM-only traffic; ACK-only and path probes, including zero-time ordinary-history placeholders and their conditional diagnostic insertion; zero/invalid intervals. Exactly-once packet delivery, explicit byte units and no division/overflow surprises. |
...
[probe-metadata]: https://github.com/the-sarge/quic-go-fast/blob/270dda4cf6a4ff66a873473e8dd68e1f12ea619f/packet_emission.go#L401-L438
...
[history-length]: https://github.com/the-sarge/quic-go-fast/blob/270dda4cf6a4ff66a873473e8dd68e1f12ea619f/internal/ackhandler/sent_packet_history.go#L132-L138
