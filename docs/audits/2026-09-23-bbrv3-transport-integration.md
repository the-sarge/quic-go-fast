...
**Prototype question:** Can registration-time sampling, explicitly bounded pacing/send quantum and compact delivery snapshots provide usable BBR timing evidence with bounded model state under queue delay, GSO, worker batching, ACK compression and application idle/resume while preserving the accepted ownership boundary? A focused experiment can resolve this question without implementing BBR or changing the public API. Its proposed contract appears below.
...
5. Detect losses among remaining entries; remove their flight bytes, queue frames for retransmission, then notify congestion for eligible losses. These loss callbacks share the loss pass's initial flight snapshot.
...
7. Detect some application-space spurious losses for diagnostics, clear the reusable slice, reset relevant PTO state and set the next loss timer.
...
`RTTStats.MinRTT` is a lifetime minimum of raw send-to-ACK duration; `LatestRTT` and smoothed RTT may subtract the eligible ACK delay. It starts with a 100ms initial estimate, can use a restored initial estimate, and resets on migration. Therefore reading `LatestRTT` after every ACK is neither a new raw measurement nor a replacement for BBR's expiring minimum filters. Preserve transport RTT for loss/PTO while defining a separate controller RTT sample/filter policy. [RTT storage and updates][rtt], [migration reset][rtt-reset]
...
| Retry | Flight zeroes; Initial/0-RTT frames are requeued; histories restart from current packet-number generators; RTT can be seeded with a minimum 5ms estimate. Controller object is retained. | Explicit sampler reset/disposal and RTT-seed rules, no duplicate delivery, no assumption that fresh history means reused packet numbers or a fresh public connection. [Retry][retry] |
| PTO extraction | First outstanding packet is declared lost in history, removed from flight and its frames requeued, without congestion-loss notification. | Give this a distinct retirement reason; no invented BBR loss ratio or TCP RTO reduction. Probe sends retain recovery's cwnd/pacing bypass. [PTO extraction][pto-extract], [send mode][sendmode], [PTO timeout][timeout] |
...
| Migration/rebinding | RTT and controller reset, outstanding application packets retire, and current code always constructs Reno. Path probes are removed separately. | Preserve opted-in algorithm identity, create a fresh path model/sampler epoch and reject stale old-model updates. [Migration][migration] |
...
Current client path replacement increments a generation, resets recovery, then replaces/drains the old queue. Server rebinding retains the active queue, and pending writes use the destination current at syscall time. Consequently, a registration's path generation is a model epoch, not conclusive proof of the packet's actual egress path. Any design claiming precise per-path send attribution must account for this queue boundary. A conservative model may discard ambiguous sampling evidence without changing the established send contract. [Connection path switch][path-switch], [server migration][path-server], [queue replacement/rebind][path-emission]
...
**Question:** Does the existing registration boundary support reliable-enough delivery evidence and a bounded BBR pacing quantum across the actual emission modes, or is an additional private queue-residence/acceptance observation necessary?

Use the smallest disposable deterministic timing model with an independent receipt/delivery oracle. Compare registration-time sampling and explicit quantum rules under queued delay, GSO/non-GSO packet groups, compressed ACKs, partial/unknown batch progress and app idle/resume. Require finite samples, exactly-once delivery, a declared bounded-state policy and explanations for rejected evidence. Socket acceptance is not actual NIC transmission. If the prototype later exercises production emission, preserve its real-packer test boundary. Native costs remain unmeasured by an abstract model. [Emission conventions][conventions]
...
| ACK event construction | Disjoint ACK ranges, duplicate/reordered ACKs, ACK plus CE plus loss, loss-only timer event, late-ACK-only event and missing history. Fixed pre/post flight, ordered valid evidence, no CE-created packet loss. |
...
| Lifecycle | Key discard, rejected 0-RTT, Retry before/after PTO, PTO extraction, close, path replacement and server rebind. No leaked metadata, stale generation update, duplicate delivery or algorithm reset to Reno after opt-in. |
...
| RTT/MTU | ACK-delay eligibility, restored/unmeasured RTT, cross-space RTT filtering, expiring BBR minima, MTU growth and handshake fallback. Recovery timers preserved and explicit controller RTT/size policy. |
