# Proposed experiment ceiling: 56 hours

Status: proposal awaiting a numerical operator decision. The enforced limit is
still 48 experiment-hours; preparation remains 16 hours and cloud cost $100.
...
| Contingency under the proposed 56-hour ceiling | 3.782 |
...
the local Mac path after the hardware and CPU-allocation decisions. These are
finite proposed allocations, not authorization to run on an unqualified path.
...
Approval would change only the overall experiment ceiling to 56 hours. Preserve
all existing reservations and invalid observations; keep the accepted 520-case
inventory, $100 cloud limit and 16-hour preparation limit. The native Mac CPU
exception and physical gateway connection remain separate prerequisites.
