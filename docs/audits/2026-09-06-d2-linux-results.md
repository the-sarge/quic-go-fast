...
Builds ran natively on minimax using Go 1.26.0, Linux 7.0.0-30-generic/amd64, Ryzen AI MAX+ 395. Each process was pinned to logical CPUs 12 and 13 (distinct physical cores) with `GOMAXPROCS=2`; their SMT siblings are 28 and 29. The governor remained `powersave` in both phases. The recorded `energy_performance_preference` was `balance_performance` throughout the ring phase and `performance` throughout the head-index phase, including every before/after snapshot. Paired comparisons use one stable setting within each phase; this difference is another reason not to compare absolute timings between phases. Go 1.26/Linux/amd64 results cannot isolate an operating-system effect relative to the earlier Go 1.27/macOS/arm64 experiment.
...
The archived `collect.py` digest in each `binary-hashes.txt` identifies the phase-local collector actually used: ring commit `597a1f63d240e4d4db7114a60a6ea6c725fc3d42` reproduces `9c3d27baf52f0dadd240c7b69b0e4ccbb2b81d2b521f40d1f38a093f1ba7b54d`; head-index commit `163392c8423c3af245d07f7bb36e9f8acb491882` reproduces `be5aa7a8bcc13815a04a68fa7a5296554cd0f2ccc96e2c4833fcefbb37fd7cf8`. The retained collector adds explicit `QUEUE_EXPERIMENT=1` to the head-index revision. The older ring revision also predates candidate-selection and partial-refill generalization. These archived hashes and the measured snapshots are preserved unchanged; they do not identify the current opt-in packaging.
