...
- **Primary — receive syscalls per delivered datagram (engaged / disabled):** geometric mean **0.9049** (0.9049001), 95 % paired-bootstrap CI **[0.8967, 0.9115]**. The fail threshold is a point ratio > 0.90; the adoption gate (upper bound ≤ 0.75) is not approached.
- **Throughput (engaged / disabled):** geometric mean **0.6708** (0.6708238), 95 % CI **[0.6678, 0.6738]** — a ~33 % regression against the ≥ 0.95 noninferiority gate (fail threshold: point < 0.90).
...
**Analysis provenance:** the declared fixed-seed paired bootstrap is implemented by [2026-09-12-d2-recvmsgx-analysis.py](2026-09-12-d2-recvmsgx-analysis.py), committed alongside these results: Python `random.Random(20260912)`, 10,000 resamples of the 10 paired rounds with replacement, geometric-mean statistic per resample, and percentile endpoints taken from the sorted resample statistics at indices `int(0.025·10000)` and `int(0.975·10000)−1`. Running it against the committed collection reproduces every interval and point estimate above.
...
- Every cell invocation transferred exactly 268,436,511 bytes; the measured receiver was delivered between 195,835 and 195,845 datagrams per invocation across all cells and rounds. The committed evidence does not attribute the small datagram-count variation to a cause.

## Why the path loses

The loopback bulk workload is **sender-bottlenecked**: the receiver outpaces the D1-batched sender. Across the counted engaged cells, 675,028 zero-progress `recvmsg_x` attempts parked on the poller against 1,097,259 delivering batch reads — 0.615 parked attempts per delivering read, and 38.1 % of the 1,772,287 visible engaged receive syscalls (the counters cannot say what fraction of logical read cycles parked at least once) — and the average delivering read collects only ~1.8 of the 8 offered messages. At that shallow fill, `recvmsg_x`'s per-call cost — the kernel internalizes and externalizes the full `msghdr_x` array and runs its multi-message receive loop — exceeds the saving from ~10 % fewer receive syscalls, and the extra receive-path CPU slows the whole in-process transfer (throughput 144.8 vs 216.4 MB/s median). The loss is syscall-path cost, not memory or allocation: the engaged cell allocates less than the disabled cell (no per-read allocation was added) and stays inside the RSS budget. Deep fills of the kind that make the D1 **send** side profitable (7.99 packets per submission, queue-accumulated) have no receive-side analogue here: arrival clumping is bounded by the sender's pacing, and a receiver that keeps up drains the queue before it deepens.
...
## Errata against the frozen protocol

The protocol was precommitted at `489b7812` and stays byte-identical; two of its statements are corrected here rather than by editing it.

- **Component-sum arithmetic:** the protocol's declared engaged per-connection model, `8 × 1452 B payload + 8 × 128 B control + ≈ 800 B scratch`, sums to **13,440 bytes ≈ 13.1 KiB**, not the "≈ 12.6 KiB" it states. The error is arithmetic only; the declared batch size, components, and the separate process-level RSS gate (the memory gate of record, which the engaged cells pass with margin) are unaffected.
- **Fixed versus capability-gated allocation:** the protocol's "≈ 1.6 KiB with the capability off" reads as if the whole engaged model were capability-gated. At the measured head `489b7812`, the eight message/OOB entries (8 × 128 B control buffers) and the eight-entry msghdr/iovec/sockaddr scratch are allocated unconditionally at connection construction; only **payload-buffer acquisition** is capability-gated (one buffer off, up to eight engaged — a 10,164-byte engaged-only increment, ≈ 13.1 KiB total under the protocol's approximate scratch term). This is a component model, not complete allocation accounting; the RSS gate disposition is unchanged.
