...
- **Topology:** the harness splits into two processes: `TestW3MeasurementServer` (Linux host, bulk sender, asserts and reports its GSO state) and `TestW3MeasurementClient` (Windows guest, measured receiver, all receiver-side metrics unchanged). The peer's GSO cell knob moves to the sender process; the sender presents an ephemeral measurement-only certificate and prints its SHA-256 fingerprint, which the orchestration passes to the receiver to pin (the two processes cannot share a regenerated certificate because Go 1.27 hedged ECDSA signing is non-deterministic, so the v1 in-process `RootCAs` pinning is preserved across processes via the fingerprint handoff rather than a shared key).
- **Superseded v1 body statements (retained below as history):** the "Fixed configuration", "Read counting", and "Collection" sections below describe the v1 hosted-runner run. v2 supersedes, specifically: the **Host** ("a GitHub-hosted `windows-latest` runner") is replaced by the minimax KVM guest above; **CPU affinity** ("not available on a hosted runner") is replaced by the pinned Linux sender above; the single-process in-process **workload** wiring and its loopback path are replaced by the two-process cross-NIC topology above; the **Read counting** wording ("the harness wraps the client's receive socket") now refers to the receiver process's `ReadMsgUDP` seam; and the **Collection** "one hosted job" is replaced by the systemd-driven per-cell orchestration on minimax. Cells, metrics, bounds, and the disposition rule are unchanged, so those v1 sentences remain accurate except for the surface/topology facts enumerated here.
...
W3 is the W track's second lander, so the four-combination offload interaction matrix is this protocol's obligation. The obligation maps to distinct receipts by ancillary type rather than one test proving everything:

- **Payload, packet-info, ECN state across all four USO×URO combinations:** `TestWindowsUSOUROInteractionMatrix` on the Windows CI matrix — for each combination, a burst of segmented (or individually submitted) datagrams must arrive intact and in order, the destination packet info must be correct on a wildcard-bound socket, and ECN must remain unsupported on the Windows conn (the Server 2022 ancillary policy — TTL/DSCP receive features stay disabled while offloads are active — is satisfied structurally because the conn requests neither feature).
- **`UDP_COALESCED_INFO` parsing and multi-segment splitting:** `TestWindowsUROReadSplitsCoalescedRead`, which drives a hand-built coalesced-info control message through the parser and asserts the segment sizes.

The matrix asserts a slab-backed view when URO is on, but that is a mode check, not a coalescing check: **every non-empty URO read is slab-backed, a single datagram included**, so slab presence does not prove the kernel coalesced. Actual coalescing engagement is owned by the adoption cells above (the engaged cell's engagement fraction), not by the matrix. The results document records the matrix and the URO closure classes passing on the exact candidate head; they remain in the ordinary suite afterward, so the matrix stays continuously enforced.
