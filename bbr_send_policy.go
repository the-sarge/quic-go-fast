...
type bbrSendPolicy struct {
	...
	tokens  uint64 // byte-nanoseconds, retaining fractional byte credit
...
func (p *bbrSendPolicy) update(rate uint64, size protocol.ByteCount, now monotime.Time) {
	...
	p.tokens = min(p.tokens, uint64(p.quantum)*1e9)
}

func (p *bbrSendPolicy) budget(now monotime.Time) protocol.ByteCount {
	if now > p.updated {
		missing := uint64(p.quantum)*1e9 - p.tokens
		delta := uint64(now.Sub(p.updated))
		if p.rate != 0 && delta >= ceilSendDelay(missing, p.rate) {
			p.tokens += missing
		} else {
			p.tokens += delta * p.rate // bounded above by missing
		}
		p.updated = now
	}
	return protocol.ByteCount(p.tokens / 1e9)
}
...
func (p *bbrSendPolicy) deadline(size protocol.ByteCount, now monotime.Time) monotime.Time {
	...
	return now.Add(time.Duration(ceilSendDelay(uint64(size)*1e9-p.tokens, p.rate)))
}
