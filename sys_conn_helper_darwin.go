...
// Common dispatch in send_conn.go references the native methods, so a missing
// sendmsg_x active/stub implementation fails compilation. Go rejects duplicate
// method definitions if the build tags overlap. This assertion documents that
// sconn also implements the platform-independent batchSender interface.
